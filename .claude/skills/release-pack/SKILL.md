---
name: release-pack
description: EasyMarkdown 的 Windows 加密 zip 打包 + 自动生成面向最终用户的中文更新日志（RELEASE_NOTES.md）。当用户提到打包、发版、出包、打个 zip、升级版本号、准备发布、release、更新日志、changelog、release notes 时都应该用这个 skill，哪怕他没明说"打包"两个字。支持版本号参数（/release-pack 1.0.14）与 --notes-only / --pack-only。仅限 Windows。
---
...
args 的第一个非 `--` token 即版本号；没有则为增量模式。
...
**head 为 `pending` 或缺失**（首次启用）：不要猜。用 **Bash 工具**（不是 PowerShell——内层引号会被吃掉）跑
`git log --oneline -S '"version": "<topVer>"' -- package.json` 拿一个候选起点，连同 `git log --oneline -30` 一起给用户挑。
...
- **不要手动 `Edit package.json`。** 版本号有 11 处落点，手改必漏。`bump` 一次改完并自检；漏改任何一处，下次的 `version:check` 会红。
