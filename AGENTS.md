...
- **Code blocks mount eagerly** (`editor-codeblock-eager.js`): patches Milkdown's
  CodeMirrorBlock prototype to skip the IntersectionObserver lazy mount + 5s
  teardown whose placeholder↔editor height delta made the page jump when
  stopping/selecting at a code block. Paired with `.editor-scroll {
  overflow-anchor: auto }`. Don't re-add `overflow-anchor: none`; keep the
  side-effect import in `Editor.jsx`.
- **CSS order: Crepe theme CSS before app.css.** `Editor.jsx` is a lazy chunk, so
  its Crepe CSS would inject AFTER `app.css` and win equal-specificity conflicts
  (Crepe's reset hardcodes `.ProseMirror p/h1–h6` font sizes → font-size slider
  dead in rich tabs). `main.jsx` imports the three Crepe theme CSS files before
  `./styles/app.css`; keep that order.
