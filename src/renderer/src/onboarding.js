...
- **Tabs** — many files in one window (\`Ctrl+Tab\` to cycle), reorderable by drag, and pinnable (pinned tabs sit on the left and survive "Close others"). When the strip overflows, a previous-tab button appears at its start and a next-tab button at its end; these buttons switch tabs, not undo/redo. Right-click a tab to close others / to the left / to the right, or to open it in a split pane on the right.
...
- **Command palette** (\`Ctrl+P\`) — fuzzy-jump to any file or command; start with \`@\` to search headings in the current document.
...
- **Links** — in Keep, plain-click a web link to open it and an internal document link to jump in-app; in Milkdown, use \`Ctrl/Cmd+Click\`. Relative-path images just work, and external edits to an open file reload automatically.
...
- **标签页** —— 一个窗口开多个文件（\`Ctrl+Tab\` 循环），可拖拽排序，也可固定（固定的标签靠左，不会被「关闭其他标签」误关）。标签多到顶栏放不下时，最前和最后会出现「上一个标签 / 下一个标签」按钮；它们用于标签迁移，不是撤销/重做。标签右键可关闭其他/左侧/右侧，或在右侧分屏中打开。
- **文件夹工作区** —— 左侧文件树，可原地新建 / 重命名 / 删除。折叠着的文件夹如有外部改动，右键「刷新」即可重新扫描。
- **命令面板**（\`Ctrl+P\`）—— 模糊跳转到任意文件或命令；输入 \`@\` 可搜索当前文档的标题。
- **大纲面板** —— 点标题跳转，随编辑实时更新。
...
- **链接** —— 保持模式下普通单击即可打开网页链接或跳到内部文档；Milkdown 下使用 **Ctrl/Cmd 点击**。相对路径图片开箱即用，外部修改正在打开的文件会自动刷新。
...
- **タブ** —— 1 つのウィンドウで複数ファイル（\`Ctrl+Tab\` で切替）。ドラッグで並べ替えでき、ピン留めも可能（ピン留めしたタブは左に寄り、「他のタブを閉じる」でも閉じません）。タブが上部に収まらないときだけ、先頭と末尾に「前のタブ / 次のタブ」ボタンが現れます。これはタブ移動用で、Undo/Redo ではありません。タブ右クリックで他を閉じる/左側を閉じる/右側を閉じる、または右側の分割ビューで開く。
- **フォルダワークスペース** —— 左のファイルツリーで作成・名前変更・削除をその場で。折りたたみ中のフォルダが外部で変更された場合は、右クリックの「更新」で再スキャンできます。
- **コマンドパレット**（\`Ctrl+P\`）—— 任意のファイルやコマンドへあいまい検索でジャンプ。\`@\` から入力すると現在の文書の見出しを検索できます。
...
- **リンク** —— キープでは通常クリックで Web リンクを開き、内部文書へ移動します。Milkdown では **Ctrl/Cmd + クリック**を使用します。相対パス画像もそのまま動作し、開いているファイルが外部で編集されると自動的に再読み込みします。
