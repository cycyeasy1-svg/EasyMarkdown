// Pure Mermaid render core — the theme-keyed LRU cache, lazy import, serialized
// render queue and retry logic, with NO Milkdown/ProseMirror dependency. Split out of
...
const canonicalSource = (code) => String(code || '').replace(/\r\n?/g, '\n').trim()
const keyFor = (theme, code) => theme + '::' + canonicalSource(code)

// Mermaid keeps mutable module-level render state, so distinct diagrams must run
// one at a time. The promise chain preserves FIFO order and keeps going after an
// individual failure. Same-source callers still coalesce via `pending`.
...
// coalesce) and queues the actual render behind the serialized queue above.
export async function ensureRender(theme, code, onDone) {
  const source = canonicalSource(code)
  const k = keyFor(theme, source)
  if (cacheGet(k)) {
    onDone?.()
    return
  }
  ...
  void queueRender(() => renderNow(k, theme, source, onDone))
}
...
async function renderNow(k, theme, code, onDone) {
  ...
  try {
    ...
    if (!retried.has(k)) {
      retried.add(k)
      const callbacks = pending.get(k) || []
      pending.delete(k)
      document.getElementById(id)?.remove()
      document.getElementById('d' + id)?.remove()
      setTimeout(() => {
        if (!callbacks.length) {
          ensureRender(theme, code)
          return
        }
        callbacks.forEach((callback) => ensureRender(theme, code, callback))
      }, 300)
      return
    }
...
export function peekMermaidSvg(code, theme = curTheme()) {
  return cacheGet(keyFor(theme, code)) || null
}
...
export function getMermaidSvg(code, theme = curTheme()) {
  const trimmed = canonicalSource(code)
