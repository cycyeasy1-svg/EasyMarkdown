...
import { renderCodeLinesHtml } from '../../../shared/code-lines.js'

const MERMAID_DEADLINE_MS = 8000
const isLatex = (language) =>
...
const codeElement = (doc, source) => {
  ...
  pre.className = 'hm-code-block'
  pre.setAttribute('data-hm-pdf-preserve', '')
  const code = doc.createElement('code')
  code.className = 'hm-code-lines'
  code.innerHTML = renderCodeLinesHtml(source)
...
const materializeTasks = (clone) => {
  clone
    .querySelectorAll('li > .label-wrapper .label.checked, li > .label-wrapper .label.unchecked')
    .forEach((label) => {
...
const stripAttributes = (clone) => {
  ...
  clone.querySelectorAll('*').forEach((element) => {
    ...
    const preserveTableStyle =
      measuredTable &&
      (element === measuredTable ||
        (element.tagName === 'COL' && element.hasAttribute('data-hm-pdf-column')))
  ...
  clone
    .querySelectorAll('[data-hm-pdf-preserve]')
...
export async function createEditorSnapshot(view, { stageImages = false } = {}) {
  ...
  for (let index = 0; index < cloneBlocks.length; index += 1) {
    ...
    const language = String(resolved?.node?.attrs?.language || '')
      .trim()
      .toLowerCase()
  ...
  const imageSources = [...root.querySelectorAll('img')].map(
