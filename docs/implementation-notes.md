...
- 代价：折叠（未加载）目录里的外部改动不被监听——但它本来就不可见，等用户展开时 `loadDir` 会重读到最新内容。用户也可以对任意目录右键 **刷新**，主动递归扫描该目录及其所有子目录的 Markdown，并把扫描结果写回 `childrenMap`（刷新中/完成都有 toast）。这是显式操作，允许它花一点时间；启动路径仍保持浅监听。

`watch:changed` 仍走 `App.onWatchChanged → refreshNonce → Sidebar 重读所有已加载目录`，行为不变；变的只是"监听谁"。这样启动只浅监听 ~3 个根 +少量祖先目录，而非 crawl 成千上万文件。

命令面板的 quick-open 索引是另一个补偿口：不会在启动时扫描全工程；每次打开命令面板时才 `fs:listFiles` 重建索引。这样折叠目录里由外部新增/删除的 Markdown 文件，即使没有目录 watch 事件，也会在下一次打开面板时进入搜索结果。面板打开期间若已监听目录发生变更，仍用 400ms debounce 刷新索引；面板关闭时不做后台全量扫描。
