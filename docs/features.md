...
- 打开多个 `.md` 在同一窗口；`Ctrl/Cmd+Tab` 显示最近使用（MRU）标签浮层，松开修饰键或按 Enter 确定，Esc 取消。
- `Ctrl/Cmd+PageUp` / `Ctrl/Cmd+PageDown` 按标签条的实际顺序切换；`Ctrl/Cmd+Shift+T` 重新打开最近关闭的文件标签。
- 文件树单击文件会放入唯一的**预览标签**，继续浏览时复用该临时位置；双击或按 `Enter` 会改为普通标签。
- 一旦编辑、固定、拖动、分屏打开或切换编辑器模式，预览标签会自动转为普通标签，不再被下一次单击替换。
- 在系统资源管理器双击 `.md` → 不新开程序，而是在已有窗口加一个普通标签。
- 新建：标签条末尾的 **+**、顶栏右侧独立的 **+** 按钮、`Ctrl+N`，都调同一个 `newTab`（新建未命名草稿，首次 `Ctrl+S` 选位置保存）
- 标签全部放得下时不显示迁移按钮；溢出时才在标签条首尾显示「上一个标签 / 下一个标签」，行为类似 Firefox。按钮只切换标签，tooltip 也明确写为标签迁移，不承担 Undo/Redo。
...
- `Tabs.jsx` 用 `ResizeObserver` 比较标签内容宽度与整个标签条宽度；只有真正溢出时才渲染 `.tab-nav-prev/.tab-nav-next`，并把当前聚焦栏切到相邻标签。
- `tab-history.js` 管理最多 20 条关闭历史、原位置插入、固定标签边界和 MRU 顺序。关闭历史随会话保存；恢复时重新读取磁盘文件并恢复固定与编辑模式。未命名草稿在确认关闭后视为明确丢弃，不进入关闭历史。
- `paths.js` 的 `openPreviewTabInList()` / `promotePreviewTabInList()` 维护唯一预览槽与显式升级；会话保存 `previewPaths`，但预览标签不会进入关闭历史。
- `Ctrl/Cmd+Shift+T` 已从主题切换改为恢复关闭标签；主题切换仍保留在「视图」菜单和命令面板。
...
- 外部增删文件会自动刷新树
- 文件树支持完整键盘操作：`↑/↓` 移动，`←/→` 收起/展开，`Enter` 打开，
  `F2` 重命名，`Delete` 删除确认，`Shift+F10` 打开上下文菜单；`Ctrl/Cmd+X`
  与 `Ctrl/Cmd+V` 可剪切并移动文件或文件夹。菜单也可用方向键、Home/End 与 Esc 操作。
- 文件树使用 `tree` / `treeitem`、roving tabindex、层级/展开/当前文件 ARIA 状态；
  焦点会自动滚动到可见区域，重命名或移动后也会跟随新路径。
...
> - 键盘移动复用同一条移动与 Markdown 链接更新路径；Windows 路径会先统一分隔符和大小写键，
>   避免重命名/移动成功后焦点因 `/`、`\` 差异丢失。
...
## 3a. 保持模式（Keep）：零差分编辑、Undo/Redo 与草稿保护

打开已有 `.md/.markdown/.mdx` 时默认使用保持模式。它直接以磁盘原文为正本，渲染层只负责显示；确认编辑时只替换对应源码行，因此不会像 WYSIWYG 序列化那样顺带改动空白、列表符号或表格对齐。

- 单击表格单元格会进入带读屏状态的选中模式；方向键、`Tab` / `Shift+Tab` 移动，`Enter` / `F2` 编辑，`Alt+↓` 筛选，`Shift+F10` 打开表格菜单，`Esc` 依次关闭弹层或退出选中。
- 双击单元格进入编辑，表头按钮打开列筛选，右键或 `Shift+F10` 打开表格菜单；不再为选中单元格显示重复的浮动工具条。行列结构操作也可从命令面板执行。
- 从 Excel 等来源粘贴 TSV 矩形数据时，会从当前单元格开始覆盖现有表格范围；超出部分明确截断，整次粘贴只生成一条 Undo 事务，不会隐式扩张表结构。
- 段落、标题、列表、引用等块可通过右上角「内容编辑」修改对应源码。
- GFM 任务框可直接切换完成状态；非表格内容块右键可在上方/下方插入、复制或删除，同样的结构操作也可从命令面板执行。
- 已确认的单元格、内容块和表格结构修改会进入每个标签独立的 Keep Undo/Redo 历史。
- `Ctrl/Cmd+Z` 撤销，`Ctrl/Cmd+Shift+Z` 重做；Windows 另支持 `Ctrl+Y`。状态栏「已修改」旁提供显式 Undo/Redo 按钮，命令面板也有对应命令。
- 操作完成提示会明确显示「单元格已修改」「已删除 1 行」等内容，并提供可直接点击的「撤销」。
- 状态栏和命令面板的「查看本次修改」会按源码行列出当前内容与上次保存版本的差异。每项可定位到正文，也可只把该范围恢复到保存版本；恢复操作本身仍会进入 Undo 历史。
- 尚未确认的单元格或块编辑作为标签内草稿保留。切换标签不会丢失；保存、切换 Keep/Milkdown 或执行 Keep Undo/Redo 时，会先要求确认或取消草稿。
- 保持模式可显示普通 Markdown、相对路径图片、Mermaid 图和块级 KaTeX 公式；APP 内的 GFM 任务框可直接点击修改 `[ ]` / `[x]`。

**实现**（`KeepEditor.jsx` + `keep-parser.js` + `keep-history.js` + `StatusBar.jsx`）：

- `KeepEditor` 以 `rawLinesRef` 保存原始行，单元格和表格结构修改优先生成最小行补丁；块编辑计算最小公共前后缀，只把变化行写入历史。
- Keep 表格以逻辑行列坐标保存当前选择，并用 roving `tabindex`、`role="grid"` / `gridcell` 与 `aria-selected` 暴露焦点状态；键盘移动只检查当前行或相邻行，不扫描整张表。
- 任务切换只替换任务所在行的标记。块插入、复制、删除以 `parseDoc` 的完整块范围为单位写入行补丁，并只在缺少边界时增加必要空行，不会拆开嵌套列表或多行引用。
- 每个历史项限制为行级 `before/after`，并记录操作类型、目标行/表格/行列、显示摘要和时间；默认最多 100 条、约 200 万字符，避免单个大表格编辑反复复制全文。
- `keep-review.js` 使用有限前瞻的行级差异算法。常见局部修改会分成可独立恢复的小块；找不到近距离同步点的大范围重写会合并成一个粗粒度差异，且列表最多 200 项，避免大表格打开审阅时产生无上限长任务。
- `KeepChangeReview.jsx` 是共用审阅界面；外部文件冲突与 Keep/Milkdown 模式切换警告也可在做决定前打开同一差异视图。外部冲突比较会重新读取磁盘最新版，并保持只读。
- `App.jsx` 记录每个标签的 `canUndo/canRedo` 与草稿状态；快捷键只在没有激活输入框时接管，草稿内仍保留 textarea 自己的文字撤销。
- Keep 与 Milkdown 仍是两种**编辑器模式**，由状态栏中的独立按钮直接切换；旁边保留原有的单个视图切换按钮。Keep 下每次点击按「富文本 → 源码 → 富文本 + 源码」循环，Milkdown 下仍按「富文本 ↔ 源码」切换，不使用下拉菜单。

## 3b. 跳转历史与阅读上下文恢复

链接、大纲、命令面板标题搜索、全文搜索等跳转进入同一份导航历史。`Alt+←` / `Alt+→` 或鼠标侧键可返回/前进；记录的不只是文档与源码位置：

- 分屏时记录跳转发生在左侧还是右侧。
- Keep 会恢复折叠标题，以及当前相关表格的筛选、横向滚动和选中单元格。
- 源码视图会恢复完整选区范围。
- 每条记录只保存一个相关表格；折叠项、筛选列和值都设有上限，临时筛选状态不会写入 Markdown。

## 3c. 同一文档「源码 + Keep」同步分栏

使用 Keep 编辑器时，可通过状态栏原有的视图按钮循环切换到「富文本 + 源码」，让同一份 Markdown 原文与 Keep 渲染同时显示。源码可放左侧或右侧，两栏沿用现有可拖动分隔条，不会因为开关或换边而重新挂载 Keep。切换到 Milkdown 后，视图按钮恢复为「富文本 ↔ 源码」两态切换。

- 源码输入以 140ms 节流刷新 Keep；Keep 已确认的任务、单元格、内容块与表格操作通过同一 `tab.content` 即时回写源码，不维护第二份文档状态。
- 双向滚动以 Markdown 字符偏移为公共坐标；可临时关闭。源码选区对应的 Keep 块会淡色标记，Keep 双击块或表格单元格会选中对应源码行。
- Keep 预览可固定到当前文档。固定后切换其他源码标签，预览不跟随；同文档恢复后可继续滚动同步。
- 未确认的 Keep 草稿仍优先受保护；草稿存在时不会让源码输入绕过确认流程覆盖它。

**实现**（`App.jsx` + `KeepEditor.jsx` + `StatusBar.jsx`）：

- `sourceSplitEnabled/sourcePreviewId/sourcePreviewPinned/sourceOnLeft/sourceScrollSync` 只描述视图布局；正文正本仍是标签的 `content`。
- `KeepEditor.syncMarkdown()` 是无历史、无回写的预览同步入口，并在源码大范围变化时清空已失效的 Keep 行补丁历史。
- 源码与 Keep 节点继续作为 `.editor-area` 的 flex 兄弟，仅修改 `display/order/flex`；E2E 用 DOM 标记验证左右互换后仍是同一个 Keep 节点。

## 3d. Zen / 集中模式

`Ctrl/Cmd+K Z` 或命令面板可切换集中模式。进入后暂时隐藏活动栏、标签栏、侧边栏和状态栏，正文、分屏、选区与未确认编辑状态都保持不变。

- 指针移到屏幕上边缘、左边缘或下边缘时，对应的原有界面会临时出现，移开后再次隐藏。
- 右下角保留一个克制的退出按钮；再次按快捷键也可退出。
- 该模式只改变应用外壳的可见性，不卸载编辑器，因此不会触发文档重解析或滚动位置变化。

## 3e. 永久本地历史

设置「文件」中可选择启用本地历史，默认关闭。启用后，每次成功保存会把**保存前的版本**留在应用数据目录，和当前会话的 Keep Undo/Redo 分开管理。

- 命令面板「打开本地历史」可查看当前文档的保存时间、保存类型和大小，并执行比较、恢复、单项删除或清空当前文档历史。
- 比较复用 Keep 修改审阅界面；完整恢复作为一条新的 Keep 事务写回当前标签，因此仍可立即 Undo，且不会直接覆盖磁盘。
- 每个文档最多 30 版、保留 30 天；自动保存 10 分钟内的连续版本会合并。单版超过 2 MB 不记录，全局最多使用 64 MB。
- 历史只保存在 Electron `userData/local-history`，不会写入原文档目录或自动上传；设置中可一键清空全部历史。
- 当前仅桌面 APP 支持。移动端因系统备份与沙盒策略不同，暂不暴露该功能。

**实现**（`src/main/local-history.js` + `LocalHistoryDialog.jsx` + `KeepChangeReview.jsx`）：

- 主进程通过 `history:add/list/read/delete/clear` IPC 读写按绝对文档路径哈希命名的 JSON 记录，并执行时间、数量、单版与全局容量清理。
- 渲染层只在用户开启设置且保存成功后提交旧版正文；读取列表时只传元数据，比较或恢复时才按需读取具体内容。
- `test/local-history.test.js` 覆盖清理与自动保存合并，`test/e2e/local-history.spec.js` 覆盖跨重启列表、比较、恢复与 Undo。
...
- 渲染层 `App.jsx` 为打开的文件挂/卸监听，收到变更后：
  - 若该标签**有未保存修改** → 不覆盖（保护你的编辑）
    - 冲突提示提供「查看差异」，重新读取磁盘最新版，与当前本地内容做只读逐段比较
  - 否则从磁盘重载，并 bump `reloadNonce` 让 Editor 重挂载
  - 忽略自己保存产生的回声（比对 mtime）

## 5. 链接打开与文档内跳转

- 保持模式是只读预览，普通单击即可打开 `http(s):` / `mailto:`，相对 Markdown 链接与 `#anchor` 则在应用内打开目标文档并跳到标题。
- Milkdown 是可编辑界面，需要按住 `Ctrl`(Win/Linux) / `Cmd`(Mac) 点击链接，避免普通点击与光标定位冲突。
- Keep 与 Milkdown 的内部链接悬停提示会显示目标文件和标题锚点；按住 `Alt/Option` 点击可直接在右侧分屏打开并跳到目标标题，Keep 右键菜单也提供该操作。
- 活动栏的「Markdown 链接」面板按需检查当前工作区：不存在的文档、图片、标题锚点，以及不允许的 URL 会按文件分组显示；`F8` / `Shift+F8` 可跳到下一个 / 上一个问题。
- 在 Keep 的标题或链接上右键，或者从命令面板执行，可「查找所有引用」；结果同样按文件分组，点击后回到精确源码行。
- 标题上按 `F2` 可预览并更新工作区内的锚点引用。标签、文件树重命名和拖拽移动 Markdown 文件时，会先展示受影响引用，允许取消、仅重命名，或重命名并更新链接。
- 跨目录移动文档时，会同时重算被移动文档内部的相对链接。所有更新只替换必要的链接目标或标题文本，不重新序列化整篇 Markdown。

**实现**：`KeepEditor.jsx` 延迟 230ms 处理普通单击，以便双击单元格/块时取消跳转；外部链接走 `shell.openExternal`，文档链接交给 `App.jsx` 解析路径与锚点。`Editor.jsx` 则在 `view.dom` 捕获阶段处理带修饰键的点击。`link-navigation.js` 统一生成内部链接的悬停提示并判断右侧打开修饰键。诊断、引用查询和更新计划集中在 `src/main/markdown-links.js`，由主进程 IPC 按需扫描；批量写入前会校验原始内容，失败时回滚已经写入的文件。
...
- 统一导航器（`Ctrl+P`）：顶部「搜索范围」下拉框可直接选择文件、命令、当前文档标题、工作区标题、跳到行号或模式帮助；熟练用户仍可输入 `>`、`@`、`#`、`:`、`?` 快速切换（`CommandPalette.jsx`）。
- 输入框只提示当前搜索范围，不再把全部文字与前缀挤在同一句中。各模式不会无条件混排；命令右侧显示快捷键，最近使用的文件与命令优先。工作区标题只在选择对应范围或输入 `#` 时由主进程延迟索引，并设置文件大小、深度、文件数与结果数上限。
- 查找（`Ctrl+F`）：文档内查找，实时显示 `当前/总数` 计数，next/prev 即时跳转
- 跳转历史：大纲、文档链接、工作区搜索结果、行号定位、文件与标签跳转统一记录；`Alt+←` / `Alt+→`、鼠标侧键或命令面板执行后退/前进。
...
**跳转历史实现**（`navigation-history.js` + `App.jsx`）：历史位置保存标签 id、源码/预览状态和视口顶部对应的 Markdown 偏移；新跳转写入 back 栈并清空 forward 栈，关闭标签后自动清理失效记录。恢复时先激活标签与对应视图，再把目标源码位置恢复到原来的视口相对位置。

**统一导航器实现**（`CommandPalette.jsx` + `workspace-headings:index` IPC）：搜索范围与搜索词分别保存；下拉选择不会向输入框写入前缀，手动输入前缀时则立即切换范围并继续搜索。文件/命令评分使用 deferred query；工作区标题模式才异步扫描 Markdown 标题，扫描每 25 个文件主动让出事件循环。结果图标使用固定的 16px flex 槽位，长路径不会再把图标压小。工作区标题与行号均复用 `jumpToTabLine`，因此也会进入导航历史。

## 9. 主题（含莫兰迪）

6 套配色：暖光、暖夜、莫兰迪·灰绿 / 豆沙 / 雾蓝 / 暮。在**设置面板**（状态栏齿轮 / `Ctrl/Cmd+,`）的「外观」区选择，也可从「视图」菜单或命令面板切换。
...
整个界面可在英文、中文、日文之间实时切换，默认跟随系统语言。在设置面板（状态栏齿轮 / `Ctrl/Cmd+,`）的「语言」区切换。

**实现**：
- `i18n-strings.js`：`STRINGS{en,zh,ja}` 翻译表；`i18n.jsx` 提供 `I18nProvider` 上下文与 `useI18n()` 的 `t(key, vars)`
...
- 点击条目打开文件；可固定常用项、移除单项，或用「清空」移除全部未固定项
...
此外，Keep 模式下的「富文本 + 源码」视图会把**同一文档**的源码与 Keep 并排显示；它与上述双文档分屏互斥，但复用相同的分隔条、比例和保持挂载机制。
...
- `editorHostRef` 指向当前活动预览，`sourceRef` 指向活动源码 textarea；`focusedTabRef` 记录最后获得焦点的栏，让保存/导出作用于你正在编辑的文档。
...
- Milkdown 的 ` ```mermaid ` 代码块默认显示**实时图表预览**，工具条可在隐藏源码 / 编辑源码之间切换，保存内容不变。
- 保持模式也直接显示 Mermaid 图；两种模式共享按主题与源码键控的 LRU 缓存，已渲染过的图可立即复用。
- 行内 `$…$`、块级 `$$…$$`(`$$` 单独成行)经 **KaTeX** 渲染;过长的显示公式在列内**横向滚动**,不溢出。
- 保持模式显示块级 `$$…$$` 公式；Mermaid、公式和图片均提供放大查看。

**实现**：Milkdown 通过 Crepe CodeMirror 自带的 `renderPreview` / `previewOnlyByDefault` 通道渲染 Mermaid，不使用 widget 装饰，也不覆盖 CodeMirror node view。`editor-mermaid.js` 动态 `import()` Mermaid，并向 Keep 暴露共享缓存的 `getMermaidSvg` / `peekMermaidSvg`。Keep 对 Mermaid 与块公式先输出占位，再由 `IntersectionObserver` 在接近视口时异步渲染；公式使用懒加载 KaTeX。Milkdown 公式启用 `CrepeFeature.Latex`，`.katex-display { overflow-x:auto }`。

## 28. 自定义主题（可迁移 Typora 主题）

把 `.css`(或整个下载来的主题文件夹)丢进**主题文件夹**,设置面板「外观」区的「自定义」主题即可选用;另有「打开主题文件夹」「获取更多主题」(theme.typora.io)。Typora 主题可**直接迁移**。

**实现**(`main` themes IPC + `customThemes.js` + `Settings.jsx`):`themes:list` **递归扫描**子目录(Typora 主题常是文件夹);`themes:read` 把相对 `url(...)` 改写成绝对 `file://`(字体/图能加载);CSS 注入到一个 `<style>`,编辑器内容带 Typora 的 `#write` / `markdown-body` 钩子;激活时(`body.hm-has-custom-theme`)正文区背景/宽度与文字 `color:inherit` 让位给主题,应用外壳保持自身风格;`applyTheme` 保留 `hm-*` body 类(切主题不丢全宽/自定义标记)。选择存于会话(`customTheme`)。
...
Keep 表格的「全部列宽自适应 / 已隐藏列」属于表格级工具条，始终位于表格框架内并随表格一起滚动。选中单元格只保留选中状态与键盘操作，不再额外显示视口浮动工具条。
...
| 关闭标签 / 循环标签 | `Ctrl+W` / `Ctrl+Tab`（反向：`Ctrl+Shift+Tab`） |
| Keep 撤销 / 重做 | `Ctrl/Cmd+Z` / `Ctrl/Cmd+Shift+Z`（Windows 也支持 `Ctrl+Y`） |
| 跳转后退 / 前进 | `Alt+←` / `Alt+→`（macOS 为 Option） |
| 命令面板 / 查找 | `Ctrl+P` / `Ctrl+F` |
| 侧边栏 / 大纲 | `Ctrl+B` / `Ctrl+Shift+L` |
| 源码视图 / 恢复关闭标签 | `Ctrl+/` / `Ctrl/Cmd+Shift+T` |
| Zen / 集中模式 | `Ctrl/Cmd+K Z` |
