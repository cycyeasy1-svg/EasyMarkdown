...
- 统一导航器（`Ctrl+P`）：不加前缀搜索最近/工作区文件；`>` 命令、`@` 当前文档标题、`#` 工作区标题、`:` 行号、`?` 模式帮助（`CommandPalette.jsx`）
- 各模式不会无条件混排；命令右侧显示快捷键，最近使用的文件与命令优先。工作区标题只在输入 `#` 时由主进程延迟索引，并设置文件大小、深度、文件数与结果数上限。
...
**统一导航器实现**（`CommandPalette.jsx` + `workspace-headings:index` IPC）：输入框只绑定即时 query，文件/命令评分使用 deferred query；`#` 模式才异步扫描 Markdown 标题，扫描每 25 个文件主动让出事件循环。工作区标题与 `:` 行号均复用 `jumpToTabLine`，因此也会进入导航历史。
