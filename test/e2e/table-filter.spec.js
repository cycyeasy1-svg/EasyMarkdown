...
test('column filter hides rows; a second column dropdown lists only surviving values', async () => {
  ...
  try {
    ...
    const red = list.locator('label', { hasText: 'red' })
    const purple = list.locator('label', { hasText: 'purple' })
    await expect(red).toHaveCount(1)
    await expect(red.locator('.km-fp-count')).toHaveText('(2)')
    await expect(purple).toHaveCount(1)
    await expect(purple.locator('.km-fp-count')).toHaveText('(1)')
