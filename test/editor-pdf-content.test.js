...
const mockView = (root, codeSource = '') => ({
  dom: root,
  posAtDOM: () => { throw new Error('CodeMirror shields DOM mapping') },
  state: {
    doc: {
      descendants(callback) {
        if (codeSource) callback({
...
describe('createEditorSnapshot', () => {
  it('uses the complete ProseMirror code node instead of virtualized CodeMirror lines', async () => {
    ...
    root.innerHTML = '<div class="milkdown-code-block"><div class="cm-editor">' +
  ...
  it('creates fixed-width image placeholders for staged PDF resources', async () => {
    ...
    root.innerHTML = Array.from({ length: 12 }, (_, index) =>
      `<img src="file:///tmp/image-${index + 1}.png">`).join('')
